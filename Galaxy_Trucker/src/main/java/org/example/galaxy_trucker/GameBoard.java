...
public class GameBoard {
    ...
    public void SetStartingPosition(String ID){
        ...
        Pair<Player, Integer> cur = players.stream().filter(p -> ID.equals( p.getKey().GetID()) ).findFirst().orElseThrow();
    ...
    private void SetNewPosition(Pair<Player, Integer> cur, int newPosition){
        ...
        positions[newPosition] = NewPair.getKey();
    ...
    public ArrayList<Player> getPlayers(){
        ...
        for(int i=0; i<nPlayers; i++) {
            PlayersCopy.add(players.get(i).getKey());
        }
