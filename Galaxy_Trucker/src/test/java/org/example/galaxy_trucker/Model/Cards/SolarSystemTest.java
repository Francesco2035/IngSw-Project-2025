...
//        TestSetupHelper.HumansSetter1(playerBoard3);
        ...
        @Test
    void cardEffect() {
        ArrayList<Goods> p1=new ArrayList<>();
        ArrayList<Goods> p2=new ArrayList<>();
        ...
        Planet Planet1=new Planet(p1);
        Planet Planet2=new Planet(p2);
        p2.add(new BLUE());
        Planet Planet3=new Planet(p2);
        ...
        SolarSystem solarSystem=new SolarSystem(1,2,GameBoard,planets);

        solarSystem.CardEffect();
            ...
            assertEquals(new ChoosingPlanet().getClass(),Passo.getPlayerState().getClass());
            assertEquals(solarSystem.getCurrentPlayer(),Passo);

            solarSystem.choosePlanet(2);
            System.out.println("sksk");
            solarSystem.choosePlanet(-1);
            solarSystem.choosePlanet(0);

            assertEquals(new Waiting().getClass(),Pietro.getPlayerState().getClass());
            assertEquals(new HandleCargo().getClass(),Franci.getPlayerState().getClass());
            assertEquals(new HandleCargo().getClass(),Passo.getPlayerState().getClass());
            solarSystem.finishCard();
            solarSystem.finishCard();
